...
class Bot
{
    public:
        ...
        void init(int timestamp_counter=1);
    ...
    // LongTermEMA function continuously calculates the EMA of either ask or bid closing prices while the bot is running. It receives the name of the product(currency) and orderbooktype and returns a long double value.
        long double LongTermEMA(std::string product, OrderBookType orderType);

    // ShortTermEMA function calculates the average of the EMAs of the 10 most recent timestamp. the name of the product(currency) and orderbooktype and returns a long double value.
